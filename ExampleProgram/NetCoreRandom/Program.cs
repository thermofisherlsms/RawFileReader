    ...
    internal static class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            { 
                ...
                // Open the RAW file using the random access methods
                var rawFileManager = RawFileReaderAdapter.RandomAccessThreadedFileFactory(filename, RandomAccessFileManager.Instance);

                // Create the IRawDataPlus object for accessing the RAW file
                using var rawAccessor = rawFileManager.CreateThreadAccessor();

                if (!rawAccessor.IsOpen)
                {
                ...
                if (rawAccessor.IsError)
                {
                    Console.WriteLine("Error opening ({0}) - {1}", rawAccessor.FileError, filename);

                    return;
                }

                // Check if the RAW file is being acquired
                if (rawAccessor.InAcquisition)
                {
                ...
                Console.WriteLine("The RAW file has data from {0} instruments" + rawAccessor.InstrumentCount);

                rawAccessor.SelectInstrument(Device.MS, 1);

                // Get the first and last scan from the RAW file
                int firstScanNumber = rawAccessor.RunHeaderEx.FirstSpectrum;
                int lastScanNumber = rawAccessor.RunHeaderEx.LastSpectrum;

                // Get the start and end time from the RAW file
                double startTime = rawAccessor.RunHeaderEx.StartTime;
                double endTime = rawAccessor.RunHeaderEx.EndTime;
                ...
                Console.WriteLine("   RAW file: " + rawAccessor.FileName);
                Console.WriteLine("   RAW file version: " + rawAccessor.FileHeader.Revision);
                Console.WriteLine("   Creation date: " + rawAccessor.FileHeader.CreationDate);
                Console.WriteLine("   Operator: " + rawAccessor.FileHeader.WhoCreatedId);
                Console.WriteLine("   Number of instruments: " + rawAccessor.InstrumentCount);
                Console.WriteLine("   Description: " + rawAccessor.FileHeader.FileDescription);
                Console.WriteLine("   Instrument model: " + rawAccessor.GetInstrumentData().Model);
                Console.WriteLine("   Instrument name: " + rawAccessor.GetInstrumentData().Name);
                Console.WriteLine("   Serial number: " + rawAccessor.GetInstrumentData().SerialNumber);
                Console.WriteLine("   Software version: " + rawAccessor.GetInstrumentData().SoftwareVersion);
                Console.WriteLine("   Firmware version: " + rawAccessor.GetInstrumentData().HardwareVersion);
                Console.WriteLine("   Units: " + rawAccessor.GetInstrumentData().Units);
                Console.WriteLine("   Mass resolution: {0:F3} ", rawAccessor.RunHeaderEx.MassResolution);
                Console.WriteLine("   Number of scans: {0}", rawAccessor.RunHeaderEx.SpectraCount);
                Console.WriteLine("   Scan range: {0} - {1}", firstScanNumber, lastScanNumber);
                Console.WriteLine("   Time range: {0:F2} - {1:F2}", startTime, endTime);
                Console.WriteLine("   Mass range: {0:F4} - {1:F4}", rawAccessor.RunHeaderEx.LowMass, rawAccessor.RunHeaderEx.HighMass);
                Console.WriteLine();

                // Get information related to the sample that was processed
                Console.WriteLine("Sample Information:");
                Console.WriteLine("   Sample name: " + rawAccessor.SampleInformation.SampleName);
                Console.WriteLine("   Sample id: " + rawAccessor.SampleInformation.SampleId);
                Console.WriteLine("   Sample type: " + rawAccessor.SampleInformation.SampleType);
                Console.WriteLine("   Sample comment: " + rawAccessor.SampleInformation.Comment);
                Console.WriteLine("   Sample vial: " + rawAccessor.SampleInformation.Vial);
                Console.WriteLine("   Sample volume: " + rawAccessor.SampleInformation.SampleVolume);
                Console.WriteLine("   Sample injection volume: " + rawAccessor.SampleInformation.InjectionVolume);
                Console.WriteLine("   Sample row number: " + rawAccessor.SampleInformation.RowNumber);
                Console.WriteLine("   Sample dilution factor: " + rawAccessor.SampleInformation.DilutionFactor);
                Console.WriteLine();
                ...
                {
                    var deviceNames = rawAccessor.GetAllInstrumentNamesFromInstrumentMethod();
                ...
                if (testInstruments)
                {
                    TestInstruments(rawAccessor);
                }

                // Display all of the trailer extra data fields present in the RAW file
                if (getTrailerExtra)
                {
                    ListTrailerExtraFields(rawAccessor);
                }

                // Get the status log items
                if (getStatusLog)
                {
                    ListStatusLog(rawAccessor, firstScanNumber, lastScanNumber);
                }

                // Get the inclusion/exclusion list
                if (getInclusionExclusionList)
                {
                    var inclusionList = GetInclusionExclusionList(rawAccessor, 1e-5);
                ...
                int numberFilters = rawAccessor.GetFilters().Count;

                // Get the scan filter for the first and last spectrum in the RAW file
                var firstFilter = rawAccessor.GetFilterForScanNumber(firstScanNumber);    
                var lastFilter = rawAccessor.GetFilterForScanNumber(lastScanNumber);
                ...
                if (getChromatogram)
                {
                    GetChromatogram(rawAccessor, firstScanNumber, lastScanNumber, true);
                }

                // Read the scan information for each scan in the RAW file
                if (readScanInformation)
                {
                    ReadScanInformation(rawAccessor, firstScanNumber, lastScanNumber, true);
                }

                // Get a spectrum from the RAW file.  
                if (readSpectrum)
                {
                    GetSpectrum(rawAccessor, firstScanNumber, false);
                }

                // Get a average spectrum from the RAW file for the first 15 scans in the file.  
                if (averageScans)
                {
                    GetAverageSpectrum(rawAccessor, 1, 15, true);
                }

                // Read each spectrum
                if (readAllScans)
                {
                    ReadAllSpectra(rawAccessor, firstScanNumber, lastScanNumber, true);
                }

                // Calculate the mass precision for a spectrum
                if (calculateMassPrecision)
                {
                    CalculateMassPrecision(rawAccessor, 1);
                }
                ...
                if (analyzeScans)
                {
                    AnalyzeAllScans(rawAccessor, firstScanNumber, lastScanNumber);
                }
                ...
                if (readAnalog)
                {
                    ReadAnalogSignal(rawAccessor, "Pump_Pressure");
                }

                // Find the mass chromatographic data for that device
                if (readMassChromatogram)
                {
                    ReadMassChromatogram(rawAccessor);
                }
                ...
                if (centroidScan)
                {
                    CentroidScan(rawAccessor, 100, true);
                }
        ...
        private static void CalculateMassPrecision(IRawDataPlus rawFile, int scanNumber)
        {
            ...
            LogEntry logEntry = (LogEntry)rawFile.GetTrailerExtraInformation(scanNumber);
